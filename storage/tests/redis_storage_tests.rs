...
use storage::{RedisStorage, State, Storage};
...
async fn test_basic_kv() {
    let mut s: RedisStorage<VersionedState> = RedisStorage::new(
        make_logger(),
        redis::Client::open("redis://127.0.0.1").unwrap(),
    )
    .await
    .unwrap();
    s.set_db(1).await.unwrap();
    s.clear_all_keys().await.unwrap();
