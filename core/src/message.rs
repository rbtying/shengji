...
impl MessageVariant {
    pub fn to_string<'a>(
        ...
        Ok(match self {
            ...
            TrickWon { winner, points: 0 } => format!("{} wins the trick, but gets no points :(", player_name(*winner)?),
            TrickWon { winner, points } => format!("{} wins the trick and gets {} points", player_name(*winner)?, points),
